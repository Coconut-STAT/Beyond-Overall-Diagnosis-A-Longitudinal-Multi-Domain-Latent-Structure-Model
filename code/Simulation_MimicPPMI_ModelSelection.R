# Simulation_MimicPPMI_ModelSelection.R
# Model selection for Mimic-PPMI (G candidates: 3-7, true G=5)
# Output: ../output/MimicPPMI_ModelSelection.Rdata, ../output/MimicPPMI_ModelSelection_G_{G}.Rdata
# Usage: Rscript Simulation_MimicPPMI_ModelSelection.R
...
result_all <- foreach(ds = 1:DS, .packages = c("Rcpp", "RcppArmadillo", "MASS", "RcppParallel", "MCMCpack", "dqrng")) %dorng% {
...

  {
    ...
    X = matrix(0, B, N * G)
    g = 1; for (b in 1:B) for (i in 1:N) X[b, i + N * (g - 1)] = XDATA[i, b + 1]
  ...
  results_for_this_ds <- list()
  
  for (set_para_num in G_values) {
    G = set_para_num
    X = matrix(0, B, N * G)
    g = 1; for (b in 1:B) for (i in 1:N) X[b, i + N * (g - 1)] = XDATA[i, b + 1]
    for (g in 2:G) for (b in 1:B) for (i in 1:N) X[b, i + N * (g - 1)] = X[b, i + N * (1 - 1)]
    ...
    res$s_e <- NULL        
    res$R_real <- R_real
    res$G_value <- set_para_num
    res$ds_id <- ds
    
    results_for_this_ds[[paste0("G_", set_para_num)]] <- res
  }
  
  return(results_for_this_ds)
}

stopCluster(cl)

save(result_all, file = paste0("../output/MimicPPMI_ModelSelection", ".Rdata"))

for (g_val in G_values) {
  result_g <- lapply(result_all, function(x) x[[paste0("G_", g_val)]])
  save(result_g, file = paste0("../output/MimicPPMI_ModelSelection_G_", g_val, ".Rdata"))

  cat(sprintf(
    "Done. Saved ../output/%s\n",
    paste0("MimicPPMI_ModelSelection_G_", g_val, ".Rdata")
))
}
