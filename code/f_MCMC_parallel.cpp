...
//         for Domain-specific Disease Progression".
// Note  : Within-chain parallel version. Uses armspp_parallel (thread-safe ARS),
//         RcppArmadillo, and RcppParallel.
//         - Model selection code commented out.
...
int f_rcategorical(const arma::vec &prob)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        if (prob[i] > remaining_prob)
        {
            success = 1;
        }
        else
        {
            success = R::rbinom(1, prob[i] / remaining_prob);
        }
...
struct logFRigWorker : public Worker
{
    const int N;
    const int G;
    const int order;
    const arma::vec &ri;
    const arma::mat &gammaL;
    const arma::mat &beta;
    const arma::mat &X;
    const arma::mat &tig;
    ...
    logFRigWorker(int N, int G, int order, const arma::vec &ri, const arma::mat &gammaL, const arma::mat &beta, const arma::mat &X, const arma::mat &tig, const arma::vec &knots, const arma::vec &Ui, arma::mat &logFRig)
        : N(N), G(G), order(order), ri(ri), gammaL(gammaL), beta(beta), X(X), tig(tig), knots(knots), Ui(Ui), logFRig(logFRig) {}

    void operator()(std::size_t begin, std::size_t end)
    {
        arma::mat bis(gammaL.n_elem / G, N * (arma::max(ri) + 1));
        ...
        for (std::size_t i = begin; i < end; ++i)
        {
            ...
            for (int g = 0; g < G; ++g)
            {
                double stash_exp = exp(arma::dot(beta.col(g).t(), X.col(i + N * g)) + Ui(i));
                ...
                {
                    double LambdatL = arma::dot(gammaL.col(g), bis.col(i + N * (t - 1)));
                    double LambdatR = arma::dot(gammaL.col(g), bis.col(i + N * t));

                    double FInterL = 1 - exp(-LambdatL * stash_exp);
                    double FInterR = 1 - exp(-LambdatR * stash_exp);
...
// [[Rcpp::export]]
arma::mat f_logFRig_parallel(int N, int G, int order, const arma::vec &ri, const arma::mat &gammaL, const arma::mat &beta, const arma::mat &X, const arma::mat &tig, const arma::vec &knots, const arma::vec &Ui)
{
    arma::mat logFRig(arma::max(ri) + 1, G * N, arma::fill::zeros);

    logFRigWorker worker(N, G, order, ri, gammaL, beta, X, tig, knots, Ui, logFRig);
...
struct PRigtWorker : public Worker
{
    const int N;
    const int J;
    const int G;
    ...
    const arma::mat &logFRig;
    int max_ri;
    int max_ri_1;

    arma::mat &PRigt;

    PRigtWorker(int N, int J, int G, const arma::vec &s, const arma::vec &ri, const arma::mat &Y, const arma::mat &Theta, const arma::mat &logFRig, arma::mat &PRigt)
        : N(N), J(J), G(G), s(s), ri(ri), Y(Y), Theta(Theta), logFRig(logFRig), PRigt(PRigt)
    {
        max_ri = ri.max();
        max_ri_1 = max_ri + 1;
    }

    void operator()(std::size_t g_start, std::size_t g_end)
    {
        arma::mat deno_log(max_ri + 1, N, arma::fill::zeros);
        for (std::size_t g = g_start; g < g_end; ++g)
        {
            arma::uvec indices = arma::find(s == g + 1);
            ...
            for (int i = 0; i < N; ++i)
            {
                ...
                arma::vec logFRig_col = logFRig.col(g + G * i).head(ri[i] + 1);
                arma::vec deno_log_col = deno_log.col(i).head(ri[i] + 1);
...
// [[Rcpp::export]]
arma::mat f_PRigt_parallel(int N, int J, int G, const arma::vec &s, const arma::vec &ri, const arma::mat &Y, const arma::mat &Theta, const arma::mat &logFRig)
{
    int max_ri = ri.max();
    int max_ri_1 = max_ri + 1;

    arma::mat PRigt(G, (max_ri_1) * N, arma::fill::zeros);

    PRigtWorker worker(N, J, G, s, ri, Y, Theta, logFRig, PRigt);
...
// [[Rcpp::export]]
arma::mat f_Psg_parallel(int N, int J, int G, const arma::vec &ri, const arma::mat &R, const arma::mat &Theta, const arma::mat &Y, const arma::vec &XI)
{
    arma::mat Psg(G, J, arma::fill::zeros);

    PsgWorker worker(N, J, G, ri, R, Theta, Y, XI, Psg);

    parallelFor(0, J, worker);
...
struct BetaWorker : public Worker
{
    ...
    const arma::mat &X;
    const double sig0;
    const double coef_range;
    const arma::mat &z;
    const arma::mat &w;
    const arma::mat &status;
    const arma::mat &LambdatR;
    const arma::mat &LambdatL;
    const arma::vec &Ui;

    BetaWorker(int N, int G, int B, arma::mat &beta, const arma::mat &X, double sig0, double coef_range, const arma::mat &z, const arma::mat &w, const arma::mat &status, const arma::mat &LambdatR, const arma::mat &LambdatL, const arma::vec &Ui)
        : N(N), G(G), B(B), beta(beta), X(X), sig0(sig0), coef_range(coef_range), z(z), w(w), status(status), LambdatR(LambdatR), LambdatL(LambdatL), Ui(Ui) {}
...
// [[Rcpp::export]]
arma::mat f_Beta_parallel(int N, int G, int B, arma::mat beta, const arma::mat &X, double sig0, double coef_range, const arma::mat &z, const arma::mat &w, const arma::mat &status, const arma::mat &LambdatR, const arma::mat &LambdatL, const arma::vec &Ui)
{
    BetaWorker worker(N, G, B, beta, X, sig0, coef_range, z, w, status, LambdatR, LambdatL, Ui);
...
struct UiWorker : public Worker
{
    ...
    UiWorker(arma::vec &Ui, int N, int G, int B, const arma::mat &beta, const arma::mat &X, double sig0, double coef_range, const arma::mat &z, const arma::mat &w, const arma::mat &status, const arma::mat &LambdatR, const arma::mat &LambdatL)
        : Ui(Ui), N(N), G(G), B(B), beta(beta), X(X), sig0(sig0), coef_range(coef_range), z(z), w(w), status(status), LambdatR(LambdatR), LambdatL(LambdatL) {}
...
// [[Rcpp::export]]
arma::vec f_Ui_parallel(arma::vec Ui, int N, int G, int B, const arma::mat &beta, const arma::mat &X, double sig0, double coef_range, const arma::mat &z, const arma::mat &w, const arma::mat &status, const arma::mat &LambdatR, const arma::mat &LambdatL)
{
    UiWorker worker(Ui, N, G, B, beta, X, sig0, coef_range, z, w, status, LambdatR, LambdatL);
...
{
    const int N, J, G, order, Lknots, max_ri;
    const arma::vec &ri;
    const arma::mat &Y;
    const arma::vec &s;
    const arma::mat &Theta;
    ...
    RcppParallel::RVector<double> L;

    LogLikelihoodWorker(int N, int J, int G, int order, int Lknots, const arma::vec &ri,
                        const arma::mat &Y, const arma::vec &s, const arma::mat &Theta,
                        const arma::mat &gammaL, const arma::vec &knots, const arma::mat &beta,
                        const arma::mat &X, const arma::mat &tig, const arma::mat &R, const arma::vec &Ui,
                        const arma::mat &logFRig, Rcpp::NumericVector &L)
        : N(N), J(J), G(G), order(order), Lknots(Lknots), max_ri(max(ri)), ri(ri), Y(Y), s(s),
          Theta(Theta), gammaL(gammaL), knots(knots), beta(beta), X(X), tig(tig), R(R), Ui(Ui), logFRig(logFRig),
          L(L) {}
                ...
                for (int j = 0; j < J; ++j)
                {
                    if (s[j] == g + 1)
                    {
                        for (int r = 0; r < R_ig - 1; ++r)
                        {
                            if (Y(j, r + max_ri * i) != 0)
                            {
                                L_local += safe_log(Theta(Y(j, r + max_ri * i) - 1, 0 + 2 * j));
                            }
                        }
                        for (int r = R_ig - 1; r < ri[i]; ++r)
                        {
                            if (Y(j, r + max_ri * i) != 0)
                            {
                                L_local += safe_log(Theta(Y(j, r + max_ri * i) - 1, 1 + 2 * j));
                            }
                        }
                    }
                }
                L_local += logFRig(R_ig - 1, g + G * i);
...
// [[Rcpp::export]]
double f_log_Likelihood_parallel(int N, int J, int G, int order, int Lknots, const arma::vec &ri, const arma::mat &Y, const arma::vec &s, const arma::mat &Theta, const arma::mat &gammaL, const arma::vec &knots, const arma::mat &beta, const arma::mat &X, const arma::mat &tig, const arma::mat &R, const arma::vec &Ui)
{
    arma::mat logFRig = f_logFRig_parallel(N, G, order, ri, gammaL, beta, X, tig, knots, Ui);

    Rcpp::NumericVector L(N, 0.0);

    LogLikelihoodWorker logLikelihoodWorker(N, J, G, order, Lknots, ri, Y, s, Theta, gammaL, knots, beta, X, tig, R, Ui, logFRig, L);

    parallelFor(0, N, logLikelihoodWorker);
...
List f_mcmc(int IR, int BI, int N, int J, int G, int B, int order, int Lknots, arma::vec ri, arma::vec dj, arma::mat Y, arma::vec s, arma::vec knots, arma::mat Theta, arma::mat Yijtc, arma::mat beta, arma::mat gammaL, arma::vec eta, arma::mat X, arma::mat tig, arma::vec Ui, double sig0, double coef_range_beta, double coef_range_ui, double a_eta, double b_eta, double SigmaG, int sigma_alpha, double sigma_beta, int ds)
{
    ...
    logFRig = f_logFRig_parallel(N, G, order, ri, gammaL, beta, X, tig, knots, Ui);
    PRigt = f_PRigt_parallel(N, J, G, s, ri, Y, Theta, logFRig);
    ...
    for (int ir = 0; ir < IR; ++ir)
    {
        ...
        sg = f_sg(J, G, s);
        XI = f_XI(sg);
        Psg = f_Psg_parallel(N, J, G, ri, R, Theta, Y, XI);
        ...
        beta = f_Beta_parallel(N, G, B, beta, X, sig0, coef_range_beta, z, w, status, LambdatR, LambdatL, Ui);
        ...
        Ui = f_Ui_parallel(Ui, N, G, B, beta, X, SigmaG, coef_range_ui, z, w, status, LambdatR, LambdatL);
        ...
        // if (ir + 1 >= BI)
        // {
        //     Ulog_py.row(ir - BI + 1) = f_log_py_parallel(N, J, G, order, Lknots, ri, Y, s, Theta, gammaL, knots, beta, X, tig, R, Ui).t();
        //     PRigt_e += PRigt;
        // }
    ...
    LogLikelihood = f_log_Likelihood_parallel(N, J, G, order, Lknots, ri, Y, mode_S, mean_Theta, mean_gammaL, knots, mean_beta, X, tig, mode_R, mean_Ui);

    // ---- Model selection (commented out) ----
    // pd = f_pd(N, J, G, Lknots, B, dj);
    // BIC = f_BIC(N, IR - BI + 1, pd, LogLikelihood);
    // AIC = f_AIC(N, IR - BI + 1, Ulog_py, pd);
    // WAIC = f_WAIC(N, IR - BI + 1, Ulog_py);
